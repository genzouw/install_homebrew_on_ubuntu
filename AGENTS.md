...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、本 PR で変更したすべてのファイル (新規追加ファイルおよび既存ファイルへの変更を含む) に含まれていない。
