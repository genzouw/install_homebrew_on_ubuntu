...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/install_homebrew_on_ubuntu`](https://github.com/genzouw/install_homebrew_on_ubuntu) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は **MUST NOT** です。ただし、事前に第5節の例外申請プロセスに従いリポジトリオーナー (@genzouw) の承認を得ている場合はこの限りではありません (詳細は第5節を参照)。承認を得ないまま提出された PR は、内容の良し悪しにかかわらず自動的にクローズされます。

### 1.1 MUST NOT — 未承認のまま含まれるとクローズされます
...
**注記**: 「✳️」印の項目は、第5節の例外申請プロセスに従い事前承認を得た PR には適用しません。その場合は、承認元の Issue 番号を PR 本文に明記してください (第5節参照)。

- [ ] ✳️ 追加するサービスが「公開 OSS リポジトリで完全無料で利用可能」であることを、**公式の料金ページ / ドキュメントの URL** で証明している。
- [ ] ✳️ LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、本 PR で変更したすべてのファイル (新規追加ファイルおよび既存ファイルへの変更を含む) に含まれていない。
- [ ] ✳️ 「無料枠内に収まる前提」の利用ではなく、「課金が一切発生しない構成」であることを PR 本文に明記している。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **MUST** 取得してください。承認を得た例外は、承認元の Issue 番号を PR 本文に明記した上でマージ対象になります。承認のない有料サービス導入 PR は自動的にクローズされます。
...
[agents.md 規格](https://agents.md/) の仕様上、エージェントは作業対象リポジトリ内の最も近い `AGENTS.md` のみを自動的に読み込みます。そのため、本ポリシーはこのファイルが配置されているリポジトリ (`genzouw/install_homebrew_on_ubuntu`) 内でのみ自動的に有効です。

[@genzouw](https://github.com/genzouw) が公開している他の公開リポジトリに同じ方針を適用したい場合は、本ファイルの内容を当該リポジトリのルートにも `AGENTS.md` として個別に配置してください。本ファイルを 1 つ配置するだけで、全公開リポジトリに自動的に適用されるわけではありません。
